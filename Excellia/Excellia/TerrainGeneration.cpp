...
std::vector<int> TerrainGeneration::generate_heights(int map_width, float epsilon, int height, long int seed)
{
	Random r(seed);
	sf::Vector2f first(r.random(0, 1), r.random(0, 1));
	...
	for (int i = 1; i < map_width; i++) {
		...
		float ny = r.random(Utils::clip(heights[i - 1].y - epsilon, 0.0f, 1.0f), Utils::clip(heights[i - 1].y + epsilon, 0.0, 1.0f));
		sf::Vector2f nh(nx, ny);
		heights.push_back(nh);
	}
...
std::vector<int> TerrainGeneration::generate_dirt(std::vector<int>& heights, int height_limit, long int seed)
{
	Random r(seed);
	std::vector<int> output;
	for (size_t i = 0; i < heights.size(); i++) {
		...
		// Clip dirt_height to stay within the valid range of the image
		dirt_height = std::max(0, std::min(dirt_height, height_limit - 6));
		int dirt_range = static_cast<int>(r.random(3, 6));
		output.push_back(dirt_range);
	}
...
std::vector<Vector2f> TerrainGeneration::generate_caves(Vector2f min, Vector2f max, long int seed)
{
	Random r(seed);
	std::vector<Vector2f> caves;
	for (int i = 0; i < 10; i++) {
